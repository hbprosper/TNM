...
* 
* The following 6 lines of C code need to be compiled separately (with 
* "* "'s removed) using a C compiler and linked with the resulting 
* Fortran compiles of a driver program, this file (BAYESPACK), and files 
* for ADBAYS, RNRTMX, HRMSYM and RANRTH. Note: some compilers (e.g. HP) 
* will require the removal of the "_" (underscore) character from 
* usrfnc_, meansb_ and bzlpnt_ for a successful compilation. 
* 
* static double ((*cfun)());
* static void ((*cmean)());
* double usrfnc_( x ) double *x; { return (*cfun)(x); }
* void meansb_( x, fvl ) double *x, *fvl; { cmean( x, fvl ); }
* bzlpnt_( cfunp, cmeanp ) void *cfunp,*cmeanp; 
*             { cfun = cfunp; cmean = cmeanp; }
* 
* Assuming the C function is in bayesupc.c, ADBAYS, RNRTMX, HRMSYM, 
* BAYTST and BAYESPACK are in adbayes.f, rnrtmx.f and hermit.f, baytst.f 
* and bayesup.f, respectively, here is a makefile (assuming the "* " is 
* removed from the beginning of each line) to make the run file baytst.out:
*
* #beginning of makefile
* OBJECTS = bayesupc.o bayesup.o adbayes.o hermit.o rnrtmx.o ranrth.o baytst.o
* .f.o: ; f77 -c $<
* .c.o: ; cc -c $<
* baytst.out: ${OBJECTS} ; f77 ${OBJECTS} -o baytst.out
* #end of makefile
*
* A direct compile directly can be done using 
*
* cc -c bayesupc.c
* f77 bayesupc.o bayesup.f adbayes.f hermit.f rnrtmx.f ranrth.f baytst.f
*
* to produce the default run file a.out.
*
*
*
      SUBROUTINE BAYSNT( M, MAXVLS, USRLGP, MN, USRMNS, PROBLM, MU, PU )
*   
*        Simplified Driver Subroutine for BANINT
*
*     Input:
...
*                 optimization.
*      PU - integer output unit number.
*               If PU = 0, no output is produced.
*               If PU > 0, some transformation information, MEANS, final 
*                   errors and standard deviations are output to unit PU.
*               If PU < 0, modal covariance Cholesky factor, modal
*                   covariance matrix, transformation information, MEANS, 
*                   errors, final standard deviations and final 
*                   covariance matrix are output to unit |PU|.
*
*    Output:
*      MU - real M-vector for computed posterior mode.
*
      EXTERNAL USRLGP, USRMNS
      CHARACTER*(*) PROBLM
      INTEGER M, MN, ML, MAXVLS, RS, PU, NUMTRN, METHOD, INFORM
      PARAMETER ( ML = 20 )  
      DOUBLE PRECISION RELREQ, MU(*), C(ML*(ML+1)/2), NRMCON
      DOUBLE PRECISION MEANS(2*ML), ERRORS(2*ML), COVRNC(ML*(ML+1)/2)
      PARAMETER ( RELREQ = 1D-16, NUMTRN = 20 )
      RS = 0
      DO METHOD = 0, 20, 10
         CALL BANINT( M, RELREQ, MAXVLS, RS, USRLGP, MN, USRMNS, 
     &                PROBLM, MU, C, PU, NUMTRN, METHOD, 
...
*
*
      SUBROUTINE BANINT( M, RELREQ, MAXVLS, RS, USRLGP, MN, USRMNS, 
...
* 
      ...
      INTEGER NU(2,MX), I, J, IJ, IR, NCLS, INFRMP
      DOUBLE PRECISION DELTA(2,MX)
     ...
     &                LGPSMX, INFRMP, USRLGP, USRMNS )
         IF ( INFRMP .GT. 0 .AND. PU .GT. 0 )
     &        WRITE( PU, '(''  WARNING: Mode Optimization Failed !'')')
      ELSE
         CALL POSTNT( -M, METHOD, NUMTRN, C, MU, DELTA, NU,
     &                 LGPSMX, INFRMP, USRLGP, USRMNS )
      END IF
      IF ( PU .GT. 0 ) THEN
         FRMATA = ' )'
         IF ( M .GT. 7 ) FRMATA = '/(14X,7F9.5)' // FRMATA
         FRMATA = ', 7F9.5' // FRMATA
         FRMATB = ', 1PE10.3,0P' // FRMATA
         FRMATC = ' )'
         IF ( MN-M .GT. 8 ) FRMATC = '/(6X,8F9.5)' // FRMATC
         FRMATC = '( 8F9.5' // FRMATC
         IF ( RS.EQ.0 .OR. OLDMTH.NE.METHOD .OR. OLDTRN.NE.NUMTRN ) THEN
            WRITE( PU,'(/'' Numerical Integration Results for '',A)') 
     &           PROBLM
            NUCON = 1
            NULIM = NUMTRN
     ...
     &           NUCON = SQRT( ( NULIM + M )/NULIM ) 
            ...
            END DO
            WRITE( PU, '( ''  Laplace Approx. for Constant is '','
     &           //'1PE10.3 )' ) DETERM/EXP(LGPSMX)
            WRITE( PU, '('' Log Posterior Max    Mode Vector'')' ) 
            WRITE( PU, '( F13.3, 4X'//FRMATA ) 
     &           -LGPSMX, ( MU(I), I = 1,M )
            IF ( PRU .LT. 0 ) THEN
               WRITE( PU, 
     &              '(20X,''Modal Covariance Matrix Cholesky Factor'')')
               DO I = 1, M*(M+1)/2
                  COVRNC(I) = C(I)
               END DO
               DO I = 1, M
                  WRITE( PU, '( I15, 2X' // FRMATA ) I, 
     &                 ( COVRNC(J), J = I*(I-1)/2+1,I*(I+1)/2 )
               END DO
               CALL CHOLPD( M, COVRNC )
               WRITE( PU, '(25X,''Modal Covariance Matrix '')')
               DO I = 1, M
                  WRITE( PU, '( I15, 2X' // FRMATA ) I, 
     &                 ( COVRNC(J), J = I*(I-1)/2+1,I*(I+1)/2 )
               END DO
            ...
            IF ( METHOD .LT. 20 ) THEN
               WRITE( PU, '(8X,''Transformation Types'')'  )
               WRITE( PU, '( 6( I2, '':'',A4,'','', A4 ) )' ) 
     &              (I,(DNSTYP(NU(J,I),DELTA(J,I),NUNRML),J=1,2 ),I=1,M)
            END IF
            IF ( NUMTRN .EQ. 20 ) THEN
               WRITE( PU, '(8X,''Delta Scale Factors'')' )
               WRITE( PU, '( 6( I2, '':'', F4.1, '','', F4.1 ) )' ) 
     &              ( I, ( DELTA(J,I),J=1,2), I=1,M )
            END IF
            CALL INTGRT( PU, METHOD, M, NF, 0, POSTSB, RELREQ,
     &                   IR, FINEST, ESTERR, NCLS, INFORM )
            OLDTRN = NUMTRN
         END IF
         WRITE( PU, '(19X,''Means'')' )
         WRITE( PU,'(''Num Pts  Constant'',I4,6I9,(14X,7I9))' )(I,I=1,M)
      ENDIF
*
...
*
      CALL INTGRT( 0, METHOD, M, NF, MAXVLS, POSTSB, RELREQ,
     &             IR, FINEST, ESTERR, NCLS, INFORM )
...
*
         ...
         IF ( NRMCON  .GT. 0 ) THEN
            ...
            IF ( PU .GT. 0 ) THEN
               WRITE( PU, '(I7' // FRMATB )
     &              NCLS, NRMCON/EXP(LGPSMX), ( MEANS(I), I = 1, M ) 
               WRITE( PU, '(''Errests''' // FRMATB ) 
     &                 ESTERR(0)/EXP(LGPSMX), ( ERRORS(I), I = 1,M )
            END IF
            IF ( PRU .LT. 0 ) THEN
               WRITE( PU,'(''  Means   Errors   Covariance Matrix'')' )
               IJ = 0
               DO I = 1,M
                  WRITE( PU, '(F9.5,F8.5'//FRMATA ) 
     &                 MEANS(I), ERRORS(I), ( COVRNC(IJ+J), J = 1,I )
                  IJ = IJ + I
               END DO
            END IF
            IF ( MN .GT. M ) THEN
               ...
               END DO
               IF ( PU .GT. 0 ) THEN
                  WRITE( PU, '(''  Additional Means and Errors'')' )
                  WRITE( PU, '(I4,7I9/(8I9))' ) ( I, I = 1, MN - M )
                  WRITE( PU, FRMATC ) (  MEANS(I), I = M+1, MN )
                  WRITE( PU, FRMATC ) ( ERRORS(I), I = M+1, MN )
               END IF
            END IF
            INFORM = 0
         ...
         ELSE
            INFORM = 1
            IF ( PU .GT. 0 ) WRITE( PU, 
     &           '('' WARNING: Approx. Normalizing Constant <= 0!'')' )
         END IF
      END IF
      NRMCON = NRMCON/EXP(LGPSMX)
      END      
*
...
*
      ...
      IF ( PU .GT. 0 ) THEN
         IF ( METHOD .EQ. 0 ) THEN
            WRITE( PU, '(8X, ''Monte Carlo Integration Results'')' )
         ELSE IF ( METHOD .EQ. 1 ) THEN
            WRITE( PU, '(8X, ''Lattice Rule Integration Results'')' )
         ELSE IF ( METHOD .EQ. 10 ) THEN
            WRITE( PU, 
     &           '(4X, ''Subregion Adaptive Integration Results'')' )
         ELSE IF ( METHOD .EQ. 20 ) THEN
            WRITE( PU, '(6X, ''Mixed Spherical-Radial Results'')' )
         ELSE IF ( METHOD .EQ. 30 ) THEN
            WRITE( PU, '(8X, ''Gauss-Hermite Integration Results'')' ) 
         ELSE IF ( METHOD .EQ. 31 ) THEN
            WRITE( PU, '(8X, ''Modified Gauss-Hermite Results'')' ) 
         ELSE IF ( METHOD .EQ. 32 ) THEN
            WRITE( PU, '(6X, ''Stochastic Radial-Spherical Results'')' )
         ELSE IF ( METHOD .EQ. 33 ) THEN
            WRITE( PU, '(6X, ''Adaptive Radial-Spherical Results'')' )
         ELSE IF ( METHOD .EQ. 34 ) THEN
            WRITE( PU, '(6X,''Monte Carlo Radial-Spherical Results'')' )
         ELSE IF ( METHOD .EQ. 35 ) THEN
            WRITE( PU,'(6X,''Lattice Rule Radial-Spherical Results'')' )
         ELSE  
            WRITE( PU, '(6X, ''No Method'', I4, '' Implemented!'' )' ) 
     &                                    METHOD
         END IF
      END IF
      IF ( MAXCLS .GT. 0 ) THEN
...
*
*     To determine mode and modal covariance matrix for LGPOST
*
      EXTERNAL LGPOST
      INTEGER I, J, M, INFORM
      DOUBLE PRECISION X(M), COVAR(M*(M+1)/2), EPS, LGPOST, LGPTMX
      PARAMETER ( EPS = 1D-10 )
      CALL DFPMAX( M, X, EPS, LGPTMX, LGPOST, INFORM )
      LGPTMX = -LGPTMX
      IF ( INFORM .EQ. 0 ) THEN
      ...
      END IF
      END
      SUBROUTINE DFPMAX( N, P, GTOL, FP, FUNC, INFORM )
*
*     For nonlinear optimization, a BFGS variable metric algorithm
*     to maximize func(x), starting from point p, adapted 
*     from Numerical Recipes in Fortran, pp. 421-422.
*
      EXTERNAL FUNC
      INTEGER ITER, N, NMX, ITMAX, INFORM
      DOUBLE PRECISION P(*), GTOL, FP, FUNC, EPS, TOLX
      PARAMETER ( NMX = 50, ITMAX = 250 )     
      PARAMETER ( EPS = 2D-10, TOLX = 4*EPS )
      INTEGER I, J
      DOUBLE PRECISION DEN, FAC, FAD, FAE, DGMAX, XIMAX, TEST, XI(NMX),
     &     DG(NMX), G(NMX), HDG(NMX), HESSIN(NMX, NMX), PNEW(NMX) 
      INFORM = 0
      FP = -FUNC( P )
      CALL CDIFFR( N, P, FUNC, G )
      DO I = 1,N
         XI(I) = -G(I)
         DO J = 1,N
            HESSIN(I,J) = 0
      ...
      ITER = 0
 10   ITER = ITER + 1
      CALL LNSRCH( N, P, FP, G, XI, PNEW, FUNC )
      TEST = 0
      DO I = 1,N
         XI(I) = PNEW(I) - P(I)
         TEST = MAX( ABS( XI(I)/MAX( ABS(PNEW(I)), 1D0 ) ), TEST )
         DG(I) = G(I)
      END DO
      IF ( TEST .GE. TOLX ) THEN
         CALL CDIFFR( N, PNEW, FUNC, G )
         DEN = MAX( ABS(FP), 1D0 )
         TEST = 0
         DO I = 1,N
            TEST = MAX( ABS( G(I)/MAX( ABS(PNEW(I)), 1D0 ) )/DEN, TEST )
            DG(I) = G(I) - DG(I)
         END DO
         IF ( TEST .GE. GTOL ) THEN
            FAC = 0
            FAE = 0
            DGMAX = 0
            XIMAX = 0
            DO I = 1,N
               ...
               FAC = FAC + DG(I)*XI(I)
               FAE = FAE + DG(I)*HDG(I)
               DGMAX = MAX ( DGMAX,  ABS(DG(I)) )
               XIMAX = MAX ( XIMAX,  ABS(XI(I)) )
            END DO
            IF ( ABS(FAC) .GT. EPS*DGMAX*XIMAX ) THEN
               FAC = 1/FAC
               FAD = 1/FAE
               DO I = 1,N
     ...
     &                           - FAD*HDG(I)*HDG(J) + FAE*DG(I)*DG(J)
      ...
      FP = -FP
      END
      SUBROUTINE LNSRCH ( N, XOLD, F, G, P, X, FUNC )
*
*     Line search for minimum of -func(x) in direction p, 
*     adapted from Numerical Recipes in Fortran, pp. 378-379.
*
      EXTERNAL FUNC
      INTEGER N, I
      DOUBLE PRECISION F, STPMAX, G(N), P(N), X(N), XOLD(N),
     &     FUNC, ALF, TOLX
      PARAMETER ( TOLX = 1D-15, ALF = 1D-4, STPMAX = 2 )
      DOUBLE PRECISION A, ALAM, ALAM2, ALAMIN, B, DISC, FOLD,
     &     F2, FOLD2, RHS1, RHS2, SLOPE, PMAX, TEST, TMPLAM
      PMAX = 0
      FOLD = F
      DO I = 1,N
         PMAX = MAX( PMAX, ABS(P(I)) )
      END DO
      IF ( PMAX .GT. STPMAX ) THEN
         DO I = 1,N
            P(I) = P(I)*STPMAX/PMAX
         END DO
      END IF
      ...
      DO I = 1,N
         SLOPE = SLOPE + G(I)*P(I)
         TEST = MAX( ABS( P(I)/MAX( ABS(XOLD(I)), 1D0 ) ), TEST )
      END DO
      ALAMIN = TOLX/MAX( TEST, 1D0 )
      ALAM = 1
 10   DO I = 1,N
         X(I) = XOLD(I) + ALAM*P(I)
      END DO
      F = -FUNC( X )
      IF ( ALAM .GE. ALAMIN .AND. F .GT. FOLD + ALF*ALAM*SLOPE ) THEN
         IF ( ALAM .EQ. 1 ) THEN
         ...
         ELSE
            ...
            ELSE
               DISC = B*B - 3*A*SLOPE
               TMPLAM = ( - B + SQRT(ABS(DISC)) )/( 3*A )
            END IF
         END IF
         ALAM2 = ALAM
         F2 = F
         FOLD2 = FOLD
         ALAM = MAX( MIN( TMPLAM, ALAM/2 ), ALAM/10 )
         GO TO 10 
      END IF
      END 
      SUBROUTINE CDIFFR( N , P, FUNC, G )
*
*     For central difference ( with extrapolation ) approximation 
*     to the gradient g of -func(p). 
*
      INTEGER N, I
      DOUBLE PRECISION P(N), G(N), FUNC, H, PT, FM, FP, FD, HI
      PARAMETER ( H = 1D-5 )
      DO I = 1,N
         PT = P(I)
         HI = MAX( ABS(PT)*H, H ) 
...
*
      EXTERNAL FUNCTN
      INTEGER M, N, NF, MAXPTS, NCLS, I, K
      DOUBLE PRECISION   
...
*
     ...
     & 0.2196441, 0.7848351, 0.4096100, 0.9807353, 0.2689915, 0.5140357/
      ...
      J = 24 - MOD( 25-J, 24 )
      END
      DOUBLE PRECISION FUNCTION NORRAN()
*
*     NORRAN generates normal random numbers with zero mean and unit
...
*
      ...
      DOUBLE PRECISION XT, XX, Y, UNIRAN
     ...
     &  0.2671391590320836D+01,  0.2776994269662875D+01,
     &  0.2776994269662875D+01,  0.2776994269662875D+01,
     &  0.2776994269662875D+01/
      Y = UNIRAN()
      J = MOD( INT( TN*UNIRAN() ), N )
      XT = X(J+1)
      NORRAN = ( Y + Y - 1 )*XT
      IF ( ABS(NORRAN) .GT. X(J) ) THEN
         XX = B*( XT - ABS(NORRAN) )/( XT - X(J) )
         Y = UNIRAN()
         IF ( Y .GT. C - AA*EXP( -XX**2/2 ) ) THEN
            NORRAN = SIGN( XX, NORRAN )
         ELSE
            IF ( EXP(-XT**2/2)+Y/(TWOPIS*XT).GT.EXP(-NORRAN**2/2) ) THEN
 10            XX = XDN*LOG( UNIRAN() )
               IF ( -2*LOG( UNIRAN() ) .LE. XX**2 ) GO TO 10
               NORRAN = SIGN( X(N) - XX, NORRAN )
            END IF
         END IF
      END IF
      END
...
*     
      INTEGER I, N, NO
      DOUBLE PRECISION P, PO, TWO, R, RO, RP, KN, PHINV, TCHNVC
...
*
     ...
     &                   SUMFUN, FUNVAL, ALPHA, X )
      EXTERNAL FUNSUB
      INTEGER NDIM, NPTS, K, J, I, NF
      DOUBLE PRECISION VK(*), ONE, UNIRAN 
      PARAMETER ( ONE = 1 )
